class Calculator {
  constructor() {
    ...
    this.numbers.forEach((number) =>
      number.addEventListener('click', this.addNumber)
    );
    this.options.forEach((option) =>
      option.addEventListener('click', this.addOption)
    );
    this.operators.forEach((operator) =>
      operator.addEventListener('click', this.calculate)
    );
  ...
  addOption = (e) => {
    ...
    if (this.option === 'dot') {
      ...
      this.result.value.length === 1
        ? (this.result.value = '0')
        : (this.result.value = this.result.value.substring(
            0,
            this.result.value.length - 1
          ));
    }
  ...
  calculate = (e) => {
    ...
    if (this.operator === 'pow') {
      ...
      this.result.value = parseFloat(
        ((this.currentResult * this.value) / 100).toPrecision(14)
      );
    else {
      if (
        this.operator === 'equal' &&
        ...
        this.currentResult = Calculations.doMath(
          this.currentResult,
          this.value,
          this.lastOperator
        );
        ...
        if (this.newNumber) {
          ...
          this.calculations[this.calculations.length - 1] =
            Calculations.returnOperator(this.operator);
          this.subRes.value = this.calculations.join(' ');
          this.reset = false;
          return;
